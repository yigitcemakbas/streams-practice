...
public class LegoSetInterfaceImpl implements LegoSetInterface{
    ...
    public LongSummaryStatistics getSummaryStatisticsOfPiecesByTheme(String theme) {
        return getLegoSets().stream()
                .filter(legoSet -> legoSet.theme().equals(legoSet.theme()))
                .collect(summarizingLong(LegoSet::pieces));
    }
    ...
    public Double getAvgPiecesOfIcons() {
        return getLegoSets().stream()
                ...
                .collect(averagingInt(LegoSet::pieces));
    }
    ...
    public Map<String, Integer> getSumOfPiecesByTheme() {
        return getLegoSets().stream()
                ...
                .collect(groupingBy(LegoSet::theme,summingInt(LegoSet::pieces)));
    }
    ...
    public Map<String, Map<String, Set<LegoSet>>> getLegoSetByThemeThenBySubtheme() {
        return getLegoSets().stream()
                .filter(legoSet -> legoSet.theme()!=null)
                .collect(groupingBy(LegoSet::theme,groupingBy(LegoSet::subtheme,toSet())));
    }
}
